...
function Uninstall-MsiByProductCode {
    ...
    Write-Host "Attempting to uninstall MSI with Product Code: $ProductCode"

    try {
        Start-Process "msiexec.exe" -ArgumentList "/x {$ProductCode} /qn /norestart" -Wait -PassThru | Out-Null
        Write-Host "Uninstallation initiated successfully for Product Code: $ProductCode"
    }
    catch {
        Write-Error "Failed to uninstall MSI with Product Code: $ProductCode. Error: $($_.Exception.Message)"
    }
}


Remove-App-EXE-S-QUOTES "Dell Pair"                                                #working
Uninstall-MsiByProductCode -ProductCode "3F2A9AE0-4FB2-41C7-A9DF-611E6FAC2B31"     #working
